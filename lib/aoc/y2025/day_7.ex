defmodule AOC.Y2025.Day7 do
  ...
  def load_data(data, _opts) do
    ...
    {tachyon_pos, splitters, m, n}
  end

  @impl true
  def part_one({tachyon_pos, splitters, m, _}, _opts) do
    1..(m - 1)
  ...
  @impl true
  def part_two({tachyon_pos, splitters, m, _}, _opts) do
    1..(m - 1)
    |> Enum.reduce({%{tachyon_pos => 1}, 0}, fn row, {beams, splits} ->
      {new_beams, add_splits} = calculate_beams(beams, splitters, row)
      {new_beams, splits + add_splits}
  ...
  defp calculate_beams(cur_beams, splitters, row, beams, splits) do
    {{_, c} = key, f} = Enum.at(cur_beams, 0)
    tail = Map.delete(cur_beams, key)
    ...
    new_beams = keys |> Enum.reduce(beams, fn key, acc -> Map.update(acc, key, f, &(&1 + f)) end)
